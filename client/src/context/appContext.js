...
function AppProvider({ children }) {
  ...
  const DeleteAppointmnetById = async (id, AppontmentPatientId) => {
    try {
      await authFetch.delete(`/rendezVous/${id}`);
      await getAppointmentsByPatientId(AppontmentPatientId);
    } catch (error) {
      console.log(error.response);
    }
  };
  ...
  const getNotificationsByUser = async () => {
    dispatch({ type: NOTIFICATIONS_BEGIN });
    try {
      const response = await authFetch(`/notifications`);
      const data = response.data;
      dispatch({ type: NOTIFICATIONS_SUCCES, payload: { data } });
    } catch (error) {
      console.log(error.response);
    }
  ...
  const DeleteAllNotifications = async () => {
    dispatch({ type: NOTIFICATIONS_BEGIN });
    try {
      await authFetch.delete(`/notifications/delete`);
      dispatch({ type: DELETE_NOTIFICATIONS_SUCCES });
    } catch (error) {
      console.log(error.response);
    }
  };
